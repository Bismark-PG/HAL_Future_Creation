...
状态：C++ 路径已实现、编译和无输入双进程烟测通过；待团队完成实际驾驶、发射及重复事件验证后验收
...
UE 5.8 的 `UEditorPerformanceSettings` 默认启用 **Use Less CPU when in Background**，并在 Editor 非前台时节流。客户端窗口获得焦点后，Editor 主机可能被节流；这是与日志吻合的首要待验证原因。团队下次先在 **Editor Preferences → Performance** 搜索并关闭 **Use Less CPU when in Background**，重启双窗口测试；`hal.VehicleNetLog 2` 只短时开启核对输入与服务器物理帧，然后改回 `0`，避免逐包日志干扰性能。若主机物理帧仍明显慢于每秒 60 帧，再使用两个独立 `-game` 进程复测并分别保存日志，以区分 Editor 节流与代码/物理性能问题。客户端本地无预测导致的可见校正仍属于阶段 4；复测前不接受“龟速是正常阶段 3 表现”的结论。
